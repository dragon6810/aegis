...
void engine::sv::Server::ProcessHandshake(const packet::clsv_handshake_t* packet, const uint8_t addr[4])
{
    ...
    for(i=0; i<MAX_PLAYER; i++)
    {
        ...
        if(this->clients[i].netchan.port != ntohs(packet->port))
            continue;
    ...
    // send a response packet, even on a repeat
    claddr = {};
    claddr.sin_family = AF_INET;
    claddr.sin_port = packet->port;
    ...
    // don't make another client on a repeat
    if(repeat)
        return;
    ...
    newcl->netchan.port = ntohs(packet->port);

    newcl->player = Player();

    strcpy(newcl->username, packet->username);

    Console::Print("accepted client %d from %hhu.%hhu.%hhu.%hhu under the username \"%s\".\n",
        icl, addr[0], addr[1], addr[2], addr[3], packet->username);
}
