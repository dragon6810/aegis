...
void Wad::Open(const char* filename)
{
    files.push_back(fopen(filename, "rb"));
}
...
GLuint Wad::LoadTexture(const char* texturename)
{
    int f;
    FILE* fileptr;
    ...
    for(int i=0; i<lowerfilename.length(); i++)
    ...
    for(f=0; f<files.size(); f++)
    {
        ...
        for (int i = 0; i < phdr.numdirs; i++)
        {
            ...
            for (int j = 0; j < lowerentry.size(); j++)
                lowerentry[j] = std::tolower(lowerentry[j]);
