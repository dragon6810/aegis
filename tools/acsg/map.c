...
qboolean GetMapToken (qboolean crossline)
{
	char    *token_p;
...
void ParseBrush (void)
{
	...
	// read the texturedef
		...
		for (i = 0; i < 2; i++)
		{
			if (strcmp(GetMapToken(false), "["))
				Error("parsing brush");

			GetMapToken(false);
			tx.vecs[i][0] = atoi(token);
			GetMapToken(false);
			tx.vecs[i][1] = atoi(token);
			GetMapToken(false);
			tx.vecs[i][2] = atoi(token);
			GetMapToken(false);
			tx.offsets[i] = atoi(token);
		}
		GetMapToken (false);
		shift[0] = atoi(token);
		GetMapToken (false);
